class_name HeroElement extends Control
...
func _ready() -> void:
	assert(
		backgrounds.size() == Hero.Rarity.size(),
...
func init(_hero: Hero) -> void:
	hero = _hero
	nine_patch_rect.texture = backgrounds[hero.rarity]
	#texture_rect.texture = hero.texture
